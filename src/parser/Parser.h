...
namespace dpl{
	...
	public:
		...
		inline auto parse(const std::string& str) {
			return parse(static_cast<std::string_view>(str));
		}

		auto parse(std::filesystem::path dir) {
			dpl::FileStream src{ dir };
			return parse(src);
		}

		std::pair<ParseTree, std::vector<Error>> parse(dpl::TextStream& src) {
			parse_init();
